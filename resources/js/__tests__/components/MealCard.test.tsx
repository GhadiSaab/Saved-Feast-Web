...
describe('MealCard', () => {
  ...
  it('shows image when available', () => {
    ...
    expect(image).toHaveAttribute('src', expect.stringContaining('/storage/meals/test-image.jpg'));
  });
  ...
  it('opens meal detail modal when card is clicked', () => {
    ...
    expect(screen.getByText('Price')).toBeInTheDocument(); // Modal price section
    expect(screen.getByText('Restaurant:')).toBeInTheDocument(); // Modal restaurant info
    expect(screen.getByText('Pickup Time:')).toBeInTheDocument(); // Modal pickup info
  });

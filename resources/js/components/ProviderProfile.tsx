...
const ProviderProfile: React.FC = () => {
    ...
    useEffect(() => {
        const fetchProviderProfile = async () => {
            ...
            setError(null);
            try {
                const response = await axios.get<ProviderProfileData>('/api/provider/profile', {
    ...
    const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const response = await axios.post<{ message: string, profile_picture_url: string }>(
                '/api/provider/profile/picture',
                formData,
                {
                    headers: {
    ...
    if (!profileData) {
        return <div className="alert alert-warning">Could not load provider profile data.</div>;
    }
    ...
    const chartOptions = {
        ...
        plugins: {
         ...
         scales: { // Needed for Bar chart, optional for Pie
            y: {
                beginAtZero: true
            }
        }
                ...
                <div className="col-md-4">
                    <div className="card mb-3">
                        ...
                        <div className="card-body text-center">
                            ...
                            {uploadError && <p className="text-danger mt-2 small">{uploadError}</p>}
                        </div>
                    </div>
                     <div className="card">
                        <div className="card-body">
                            ...
                            <p><strong>Website:</strong> {profileData.website ? <a href={profileData.website} target="_blank" rel="noopener noreferrer">{profileData.website}</a> : 'N/A'}</p>
                            <p><strong>Description:</strong> {profileData.description || 'No description provided.'}</p>
                        </div>
                    </div>
                </div>
                            ...
                            <div className="row">
                                ...
                                <div className="col-md-4 d-flex flex-column align-items-center">
                                     ...
                                     <div style={{ maxWidth: '200px', maxHeight: '200px' }}>
                                        <Pie options={{ ...chartOptions, plugins: { ...chartOptions.plugins, title: { display: false } } }} data={pieChartData} />
                                     </div>
                                </div>
                            </div>
                        </div>
